...
[%collapsible%open]
====
L'objectif de cette solution est de démontrer la protection des données des machines virtuelles s'exécutant dans les domaines de charge de travail VMware vSphere, VCF VI ou VCF. Les machines virtuelles de cette solution sont hébergées sur des datastores NFS fournis par NetApp ONTAP. Cette solution suppose que les composants suivants sont configurés et prêts à l'emploi :
