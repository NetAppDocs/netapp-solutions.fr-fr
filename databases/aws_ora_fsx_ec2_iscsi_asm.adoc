...
[%collapsible]
====
Une fois les prérequis provisionnés, connectez-vous à l'instance EC2 en tant qu'utilisateur root pour configurer le noyau Linux pour l'installation d'Oracle.
...
[%collapsible]
====
Provisionnez trois volumes à partir de la console FSX pour héberger les fichiers binaires, de données et de journaux de la base de données Oracle.
...
. Connectez-vous à l'instance EC2 via SSH en tant qu'utilisateur ec2. Ensuite, modifiez votre clé SSH et votre adresse IP d'instance EC2.
+
....
...
....
. Changer la propriété des périphériques en `oracle:oinstall`.
+
....
...
....
. En tant qu'utilisateur Oracle, lancez `gridSetup.sh` pour l'installation de l'infrastructure de grille.
+
....
...
....
. Si vous avez reçu le message « [INS-43080] certains assistants de configuration ont échoué, ont été annulés ou ignorés », exécutez la commande suivante en tant qu'utilisateur oracle pour vérifier si la ressource du groupe de disques DE DONNÉES a été créée.
+
....
bin/crsctl stat res -t
....
. Si le groupe de disques DE DONNÉES n'a pas été créé, rechargez le multipathd en tant qu'utilisateur root.
+
....
systemctl restart multipathd
....
. En tant qu'utilisateur oracle, créez un groupe de disques DE DONNÉES s'il n'a pas été créé lors de l'installation du grid.
+
....
...
....
. Exécuter l'installation silencieuse de la base de données logicielle uniquement.
+
....
 ./runInstaller -applyRU /tmp/archive/34765931/ -silent -ignorePrereqFailure -responseFile /tmp/archive/dbinstall.rsp
....
. Exécutez le `root.sh` script après l'installation du logiciel uniquement.
+
....
/u01/app/oracle/product/19.0.0/db1/root.sh
....
. Créer le `dbca.rsp` fichier avec les entrées suivantes :
+
....
...
totalMemory=8192
....
. Création de base de données de lancement avec dbca.
+
....
...
....
. Valider les services Oracle Restart HA après la création de la base de données.
+
....
