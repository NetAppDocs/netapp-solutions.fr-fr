...
. Ouvrez la console Amazon FSX à l'adresse https://[]
...
. Créez des volumes en suivant l'étape indiquée dans https://["Création d'un volume sur FSX pour NetApp ONTAP"^].
+
|===
...
. Pour obtenir un alignement des E/S avec le schéma de partitionnement du système d'exploitation, utilisez Windows_2008 comme type de LUN recommandé. Reportez-vous à https://["ici"^] pour plus d'informations.
...
|===
Pour plus d'informations, voir exigences en matière d'espace et de dimensionnement 

Pour la compatibilité de la version, voir https://["Matrice d'interopérabilité NetApp"^].
...
[%collapsible]
====
Suivez le https://["Installez le serveur SnapCenter"^] et https://["Installation du plug-in SnapCenter pour Microsoft SQL Server"^] Pour installer et configurer SnapCenter.
...
Pour automatiser la création de LUN ISCSI et l'installation de NetApp SnapCenter avec PowerShell, clonez le référentiel à partir de https://["Lien GitHub"^].
