...
---
= Tr-5006 : mise en œuvre d'une solution VLDB Oracle haut débit sur Google Cloud NetApp volumes avec Data Guard
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
NOTE: La boîte à outils est uniquement accessible à l'utilisateur interne de NetApp avec un accès au bitbucket pour le moment. Pour les utilisateurs externes intéressés, veuillez demander l'accès à l'équipe de gestion de compte ou contacter l'équipe d'ingénierie des solutions NetApp. Reportez-vous link:https://docs.netapp.com/us-en/netapp-solutions/databases/automation_ora_clone_gcnv_asm.html#purpose["Cycle de vie automatisé des clones Oracle sur GCNV avec ASM"^]à pour obtenir des instructions d'utilisation.
