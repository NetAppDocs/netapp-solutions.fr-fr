...
La boîte à outils d'automatisation suivante contient un playbook qui peut être utilisé pour créer ou actualiser des clones d'un Oracle VLDB à partir d'une copie d'image de sauvegarde de base de données principale stockée sur le stockage GCNV via un montage NFS pour une gestion complète du cycle de vie des clones.
