...
summary: 'Cette section fournit les étapes détaillées requises pour configurer GPFS et transférer les données vers NFS à l"aide de NetApp XCP.' 
---
= Étapes détaillées de GPFS vers NFS
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
link:bda-ai-business-benefits.html["Précédent : avantages pour l'entreprise."]

[role="lead"]
Cette section fournit les étapes détaillées requises pour configurer GPFS et transférer les données vers NFS à l'aide de NetApp XCP.
...
…
....
. Installez le pack prérequis (y compris les en-têtes du chef et du noyau) sur tous les nœuds.
+
....
...
....
. Désactivez SELinux sur tous les nœuds.
+
....
...
[root@mastr-51 5.0.3.1]#
....
. Configurez le nœud d'installation.
+
....
...
[root@mastr-51 installer]#
....
. Ajoutez le nœud admin et le nœud GPFS au fichier de définition de cluster.
+
....
...
[root@mastr-51 installer]#
....
. Ajoutez l'interface graphique, les nœuds d'administration et GPFS.
+
....
...
[root@mastr-51 installer]#
....
. Ajouter un autre serveur d'interface graphique.
+
....
...
....
. Spécifiez le binaire du shell distant à utiliser par GPFS ; utiliser `-r argument`.
+
....
...
[root@mastr-51 installer]#
....
. Vérifiez d'abord les configurations avant l'installation.
+
....
...
[root@mastr-51 cluster-test]#
....
. Vérifiez que GPFS est lu et écrit en exécutant le `dd` commande.
+
....
...
Pour exporter GPFS dans NFS, procédez comme suit :
...
[root@mastr-51 ~]#
....
. Répertorier les fichiers dans GPFS pour valider le client NFS.
+
....
...
. Installez les modules dans le client NFS.
+
....
...
[root@hdp2 ~]#
....
. Validez la liste des fichiers GPFS dans le dossier monté sur NFS.
+
....
...
[root@hdp2 gpfstest]#
....
. Déplacer les données du NFS GPFS- exporté vers le NFS NetApp en utilisant XCP.
+
....
...
link:bda-ai-mapr-fs-to-ontap-nfs.html["Next : de MapR-FS sur le protocole ONTAP NFS."]
