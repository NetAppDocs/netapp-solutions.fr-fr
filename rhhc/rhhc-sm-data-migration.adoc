...
[role="lead"]
Cette page présente les options de migration des données pour les workloads de conteneurs sur des clusters Red Hat OpenShift avec Trident Protect (ACC). Plus précisément, les clients peuvent utiliser Trident Protect pour : déplacer certains workloads sélectionnés ou tous les workloads de leurs data centers sur site vers le cloud ; cloner leurs applications dans le cloud à des fins de test ou passer du data Center au cloud
