...
La puissance d'Astra Control (ACS ou ACC) est NetApp Trident. Trident intègre plusieurs types de clusters Kubernetes, tels que Red Hat OpenShift, EKS, AKS, SUSE Rancher, Anthos, etc., avec diverses versions du stockage NetApp ONTAP, telles que FAS/AFF, ONTAP Select, CVO, Google Cloud Volumes Service, Azure NetApp Files et Amazon FSX pour NetApp ONTAP.
