...
Google Cloud chiffre le trafic au niveau du réseau comme décrit à la section https://["Chiffrement en transit"^] Dans la documentation Google. Comme indiqué dans la section « architecture de services Cloud volumes », Cloud Volumes Service est fourni à partir d'un projet de production PSA contrôlé par NetApp.
...
Pour NFSv4.1, CVS-Performance propose l'authentification Kerberos, comme décrit dans https://["RFC7530"^]. Vous pouvez activer Kerberos par volume.
...
Selon RedHat https://["Options de montage courantes"^] documentation :
