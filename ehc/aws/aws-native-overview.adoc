...
Amazon FSX pour NetApp ONTAP est un service entièrement géré qui offre un stockage de fichiers extrêmement fiable, évolutif, haute performance et riche en fonctionnalités basé sur le système de fichiers populaire NetApp ONTAP. Amazon FSX pour NetApp ONTAP (plusieurs zones de disponibilité) utilise une adresse IP flottante qui permet de basculer le trafic NAS en cas de défaillance au niveau de la zone de disponibilité. Cette adresse IP se trouve en dehors de l'espace d'adresse VPC CIDR et ne peut donc pas être acheminée vers le SDDC via l'ENI. Par conséquent, VMware Transit Connect doit être utilisé pour se connecter à l'adresse IP flottante de l'interface NAS.
