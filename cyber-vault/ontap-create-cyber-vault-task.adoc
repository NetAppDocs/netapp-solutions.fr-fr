...
* Les volumes source et destination doivent être créés dans des clusters associés avec des SVM peering. Pour plus d'informations, voir link:../../ontap-sm-classic/peering/index.html["Peering de clusters"^].
...
. Sur la baie de destination, pour préparer le cyber-coffre-fort, link:../../ontap/system-admin/install-license-task.html["Installez la licence ONTAP One"^], link:../../ontap/snaplock/initialize-complianceclock-task.html["Initialiser l'horloge de conformité"^]et, si vous utilisez une version ONTAP antérieure à 9.10.1, link:../../ontap/snaplock/create-snaplock-aggregate-task.html["Créer un agrégat SnapLock Compliance"^].
...
. Sur le cluster de destination, pour créer l'airGap, définissez la période de rétention par défaut, comme décrit à la section link:../../ontap/snaplock/set-default-retention-period-task.html["Définir la période de conservation par défaut"^]. Une période de conservation par défaut est affectée à un volume SnapLock cible du coffre-fort. La valeur pour cette période est initialement définie sur un minimum de 0 ans et un maximum de 30 ans pour les volumes SnapLock Compliance. À chaque copie NetApp Snapshot, toutes les copies NetApp Snapshot sont conservées pendant cette période de conservation par défaut. La période-conservation-par-défaut doit être modifiée. La période de conservation peut être prolongée ultérieurement, si nécessaire, mais jamais raccourcie. Pour plus d'informations, voir link:../../ontap/snaplock/set-retention-period-task.html["Aperçu de la durée de conservation"^].
. link:../../ontap/data-protection/create-replication-relationship-task.html["Créer une nouvelle relation de réplication"^] Entre la source non SnapLock et la nouvelle destination SnapLock que vous avez créée à l'étape 3.
+
...
+
link:../../ontap/data-protection/create-custom-replication-policy-concept.html["Création d'une règle de réplication personnalisée"^] ou un link:../../ontap/data-protection/create-replication-job-schedule-task.html["planification personnalisée"^] si les valeurs par défaut disponibles ne sont pas adaptées.
