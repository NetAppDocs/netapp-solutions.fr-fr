...
NetApp Astra Control Center peut être installé sur un cluster {k8s_distribution_name} sur lequel l'orchestrateur de stockage Trident est déployé et configuré avec les classes de stockage et les systèmes back-end de stockage sur les systèmes de stockage NetApp ONTAP.

Pour plus d'informations sur Trident, voir link:dwn_overview_trident.html["ce document ici"^].
...
* Trident doit déjà être installé et configuré sur chacun des clusters Kubernetes Tanzu.
...
* Classe de stockage par défaut configurée sur chaque cluster Kubernetes Tanzu avec Trident comme provisionneur de stockage.
