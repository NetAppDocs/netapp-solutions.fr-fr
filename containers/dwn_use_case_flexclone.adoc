...
. Système NetApp ONTAP avec un système back-end de stockage configuré via NetApp Trident.
. Ansible 2.9+.
...
Chacun de ces environnements est géré par NetApp Astra Control, les volumes persistants étant actuellement stockés sur un système de stockage NetApp ONTAP avec un back-end NFS fourni par NetApp Trident.
