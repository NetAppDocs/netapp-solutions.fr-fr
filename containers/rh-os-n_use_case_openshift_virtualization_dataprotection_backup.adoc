...
---
= Création d'une sauvegarde à la demande pour les machines virtuelles dans la virtualisation OpenShift
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
Un snapshot des volumes persistants soutenant les disques sera créé par le CSI et sera déplacé vers l'emplacement de stockage objet fourni dans le yaml. La sauvegarde restera dans le système pendant 30 jours, comme spécifié dans le ttl.
...
spec:
  ...
  ttl: 720h0m0s
....
Une fois la sauvegarde terminée, sa phase doit s'afficher comme terminée.
...
== Création de sauvegardes planifiées pour les machines virtuelles dans la virtualisation OpenShift

Pour créer des sauvegardes selon une planification, vous devez créer une ressource personnalisée planification.
...
Les espaces de noms à inclure dans la sauvegarde et l'emplacement de stockage de la sauvegarde sont également spécifiés. Par conséquent, au lieu d'une CR de sauvegarde, la CR de planification est utilisée pour créer une sauvegarde en fonction de l'heure et de la fréquence spécifiées.
