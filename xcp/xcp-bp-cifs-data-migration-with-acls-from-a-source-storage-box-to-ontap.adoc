...
summary: 'Cette section décrit la procédure détaillée de migration des données CIFS avec des informations de sécurité d"une source vers un système ONTAP cible.' 
---
= Migration des données CIFS avec listes de contrôle d'accès depuis le boîtier de stockage source vers ONTAP
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
link:xcp-bp-data-migration-from-7-mode-to-ontap.html["Précédent : migration des données de 7-mode vers ONTAP."]

Cette section décrit la procédure détaillée de migration des données CIFS avec des informations de sécurité d'une source vers un système ONTAP cible.
...
NOTE: Si il n'y a pas d'agrégat de données, créez-en un nouveau à l'aide de `storage aggr create` commande.
...
....
. Créez une LIF de données pour répondre aux demandes des clients SMB.
+
....
...
2 entries were displayed.
....
. Monter le volume de données cible dans le namespace du SVM
+
....
...
....
. Démarrer le service CIFS sur le SVM cible
+
....
...
....
. Vérifier que l'export policy par défaut est appliquée au SVM cible.
+
....
...
....
. Modifiez les règles des export-policy pour autoriser l'accès aux clients CIFS.
+
....
...
....
+
Vérifiez que les règles de stratégie sont modifiées.
...
2 entries were displayed.
....
. Connectez-vous au système client Windows sur lequel XCP est installé. Accédez au chemin d'installation XCP.
+
....
C:\WRSHDNT>dir c:\netapp\xcp
dir c:\netapp\xcp
...
....
. Interrogez les exportations SMB du nœud source en exécutant le `xcp show` Commande sur le système hôte client XCP Windows.
+
....
...
....
. Exécutez le `help` commande de copie.
+
....
...
....
. Sur le système ONTAP cible, obtenez la liste des noms d'utilisateur et de groupe local que vous devez fournir comme valeurs pour le `fallback-user` et `fallback-group` chemin des arguments.
+
....
...
5 entries were displayed
....
. Pour migrer les données CIFS avec des listes de contrôle d'accès de la source vers la cible, exécutez la `xcp copy` commande avec `-acl` et `–fallback-user/group` options.
+
Pour le `fallback-user/group` Options, spécifiez tout utilisateur ou groupe qui peut être trouvé dans Active Directory ou utilisateur/groupe local vers le système cible.
...
C:\WRSHDNT>
....
. Si `xcp copy` entraîne le message d'erreur `ERROR failed to obtain fallback security principal`, ajoutez la zone de destination dans le fichier hosts (`C:\Windows\System32\drivers\etc\hosts`).
+
Utilisez le format suivant pour l'entrée de boîte de destination du stockage NetApp.
...
....
. Si vous recevez toujours le message d'erreur `ERROR failed to obtain fallback security principal` après avoir ajouté l'entrée de boîte de destination dans les fichiers hôtes, l'utilisateur/le groupe n'existe pas dans le système cible.
+
....
...
C:\WRSHDNT>
....
. Utiliser `xcp copy` Pour migrer des données CIFS avec des listes de contrôle d'accès (avec ou sans le dossier racine).
+
...
link:xcp-bp-best-practice-guidelines-and-recommendations.html["Ensuite, des recommandations et des bonnes pratiques."]
