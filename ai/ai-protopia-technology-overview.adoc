...
link:ai-protopia-solution-areas.html["Précédent : catégories de solutions"]
...
Pour plus d'informations sur ONTAP ai, rendez-vous sur https://["NVA-1153 : NetApp ONTAP ai avec des systèmes NVIDIA DGX A100 et des switchs Ethernet Mellanox Spectrum."^]
...
* NetApp FabricPool Tiering automatique des données inactives vers des options de stockage de cloud public et privé, notamment Amazon Web Services (AWS), Azure et la solution de stockage NetApp StorageGRID. Pour plus d'informations sur FabricPool, voir https://["Tr-4598 : meilleures pratiques de FabricPool"^].
...
Reposant sur de puissants algorithmes d'IA,  https://["Le bon sens des données cloud avec NetApp"^] permet d'automatiser le contrôle et la gouvernance des données dans l'ensemble de votre environnement de données. Vous pouvez facilement identifier les économies réalisables, identifier les problèmes de conformité et de confidentialité, et trouver des opportunités d'optimisation. Le tableau de bord Cloud Data Sense vous permet d'identifier les données dupliquées pour éliminer la redondance, mapper les données personnelles, non personnelles et sensibles, et activer des alertes en cas de données sensibles ou d'anomalie.

link:ai-protopia-test-and-validation-plan.html["Suivant : plan de test et de validation."]
