...
• Provisionner rapidement de nouveaux espaces de travail JupyterLab haute capacité, pris en charge par un stockage NetApp haute performance à évolutivité horizontale.
• Provisionner rapidement les nouvelles instances du serveur NVIDIA Triton Inférence qui sont soutenues par un stockage NetApp de grande qualité.
• Cloner presque instantanément des espaces de travail JupyterLab de grande capacité afin de permettre l'expérimentation ou l'itération rapide.
• Enregistrement quasi instantané d'espaces de travail JupyterLab de grande capacité pour la sauvegarde et/ou la traçabilité/le contrôle de base.
• Provisionnez, clonez et snapshot de manière quasi instantanée des volumes de données hautes performances de grande capacité.
