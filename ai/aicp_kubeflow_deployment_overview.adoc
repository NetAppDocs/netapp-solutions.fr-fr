...
. Vous disposez déjà d'un cluster Kubernetes opérationnel, et vous exécutez une version de Kubernetes prise en charge par Kubeflow. Pour obtenir la liste des versions prises en charge, reportez-vous à la section https://["Documentation officielle Kubeflow"^].
...
NOTE: Vous pouvez également déployer Kubeflow manuellement en suivant les options https://["instructions d'installation"^] Dans la documentation officielle Kubeflow

. Déployez Kubeflow dans votre cluster en suivant les options https://["Instructions de déploiement Kubeflow"^] Sur le site GitHub NVIDIA DeepOps.
...
+
Le nom d'utilisateur par défaut est `admin@kubeflow.org`, et le mot de passe par défaut est `12341234`. Pour créer d'autres utilisateurs, suivez les instructions de la section https://["Documentation officielle Kubeflow"^].
