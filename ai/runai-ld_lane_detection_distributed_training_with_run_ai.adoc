...
Pour la configuration et l'installation du cluster AKS, accédez à https://["Créer un cluster AKS"^]. Ensuite, suivez ces étapes :
...
. Une fois le déploiement terminé, cliquez sur connexion au cluster. Pour vous connecter au cluster AKS récemment créé, installez l'outil de ligne de commandes Kubernetes à partir de votre environnement local (ordinateur portable/PC). Visitez https://["Installation des outils"^] Pour l'installer conformément à votre système d'exploitation.
. https://["Installez l'interface de ligne de commandes Azure dans votre environnement local"^].
...
Pour terminer la configuration de Azure NetApp Files, vous devez d'abord la configurer comme décrit à la section https://["Démarrage rapide : configurez Azure NetApp Files et créez un volume NFS"^].
...
. https://["Enregistré pour Azure NetApp Files et le fournisseur de ressources NetApp (via le shell cloud Azure)"^].
. https://["Créé un compte dans Azure NetApp Files"^].
. https://["Configurez un pool de capacité"^] (Minimum 4 Tio Standard ou Premium selon vos besoins).
...
Pour plus d'informations, rendez-vous sur https://["Créez, modifiez ou supprimez un peering de réseau virtuel"^].
...
. https://["Installez tout d'abord le gouvernail"^].
...
cd ~
....
. Cloner le https://["référentiel de projet"^] `lane-detection-SCNN-horovod`.
...
. https://["Installez le cluster RUN:ai sur AKS"^].
...
Vous pouvez modifier le niveau de service d'un volume existant en déplaçant ce volume vers un autre pool de capacité qui utilise le https://["niveau de service"^] vous voulez le volume. Cette modification de niveau de service existante pour le volume n'exige pas la migration des données. Elle n'affecte pas non plus l'accès au volume.
...
Le changement de niveau de service dynamique est actuellement dans l'aperçu public, mais il n'est pas activé par défaut. Pour activer cette fonction sur l'abonnement Azure, suivez les étapes indiquées dans le document « file://["Modification dynamique du niveau de service d'un volume"^]. »

* Vous pouvez également utiliser les commandes suivantes pour Azure : interface de ligne de commandes. Pour plus d'informations sur la modification de la taille du pool de Azure NetApp Files, rendez-vous sur https://["Volume az netappfiles : gestion des ressources de volume Azure NetApp Files (ANF)"^].
+
....
...
....
* Le `set- aznetappfilesvolumepool` Cmdlet affichée ici peut modifier le pool d’un volume Azure NetApp Files. Pour plus d'informations sur la modification de la taille du pool de volumes et d'Azure PowerShell, rendez-vous sur https://["Modification du pool d'un volume Azure NetApp Files"^].
+
....
